...
// ─── State ─────────────────────────────────────────────
let files = []; // { id, path, name, size, status, progress, outputPath }
...
dropZone.addEventListener('click', async () => {
  ...
  // Use native dialog to select files (reliable path retrieval)
  const selectedFiles = await window.electronAPI.selectFiles();
  if (selectedFiles && selectedFiles.length > 0) {
    addFilesFromDialog(selectedFiles);
  }
});
...
dropZone.addEventListener('drop', (e) => {
  ...
  const droppedFiles = Array.from(e.dataTransfer.files).filter(isVideoFile);
  addFilesFromDrop(droppedFiles);
});

// Hidden file input is no longer used (we use native dialog instead)
...
// Add files from drag-and-drop (uses webUtils.getPathForFile)
function addFilesFromDrop(droppedFiles) {
  for (const f of droppedFiles) {
    const filePath = window.electronAPI.getFilePath(f);
    if (!filePath) {
      console.warn('Could not get path for file:', f.name);
      continue;
    }
    // Skip duplicates
    if (files.some((existing) => existing.path === filePath)) continue;
...
// Add files from native dialog (paths already resolved)
...
function updateUI() {
  ...
  if (allDone && files.length > 0) {
    ...
    btnConvert.disabled = true;
    btnConvert.innerHTML = `
      <svg class="spin" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12a9 9 0 11-6.219-8.56"/></svg>
      Conversion en cours...
    `;
  } else {
...
function renderFileList() {
  ...
  for (const file of files) {
    ...
    switch (file.status) {
      case 'waiting':
        statusHTML = `<div class="status-badge waiting"><span class="status-dot"></span>En attente</div>`;
        break;
      case 'converting':
        statusHTML = `<div class="status-badge converting"><span class="status-dot"></span>${file.progress}%</div>`;
        break;
      case 'done':
        statusHTML = `<div class="status-badge done"><span class="status-dot"></span>Terminé</div>`;
        break;
      case 'error':
        statusHTML = `<div class="status-badge error"><span class="status-dot"></span>Erreur</div>`;
        break;
    }
    ...
    if (file.status === 'done' && file.outputPath) {
      actionsHTML = `
        <button class="btn-open-folder" data-path="${file.outputPath}" title="Ouvrir dans l'explorateur">
  ...
  // Event listeners for open folder buttons
...
window.electronAPI.onConversionProgress((data) => {
  ...
  if (file) {
    ...
    // Update just the progress bar and badge without full re-render
    ...
    if (item) {
      ...
      if (badge) {
        badge.innerHTML = `<span class="status-dot"></span>${data.percent}%`;
      }
    }
  }
});
